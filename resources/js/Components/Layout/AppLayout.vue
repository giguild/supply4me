<template>
    <div class="min-h-screen flex" :class="theme === 'dark' ? 'bg-gray-900' : ''" style="background: linear-gradient(135deg, #f5f0eb 0%, #faf8f6 50%, #f5f0eb 100%)" :style="theme === 'dark' ? { background: 'linear-gradient(135deg, #111827 0%, #1a1f2e 50%, #111827 100%)' } : {}">
        ...
        >
            <!-- Sidebar Header -->
            <div class="flex items-center gap-2 px-3 h-16 border-b border-gray-200 dark:border-gray-700 shrink-0">
                ...
                <span v-if="!sidebarCollapsed" class="font-bold text-gray-900 dark:text-gray-100 text-sm truncate">SUPPLY4ME</span>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
                <template v-for="item in mainNav" :key="item.route">
                    <Link :href="route(item.route)" :title="item.label"
                        ...
                        :class="isActive(item.route) ? 'bg-accent/10 text-accent' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100'"
                    >
                ...
                <template v-for="group in moreNavGroups" :key="group.label">
                    <div>
                        <button
                            ...
                            class="flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors w-full text-left"
                            :class="sidebarCollapsed ? 'justify-center text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700' : 'text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'"
                        >
                            <svg class="w-3.5 h-3.5 shrink-0 transition-transform duration-200" :class="expandedGroups.includes(group.label) ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                            </svg>
                            <span v-if="!sidebarCollapsed" class="truncate">{{ group.label }}</span>
                        </button>

                        <div v-show="expandedGroups.includes(group.label) && !sidebarCollapsed" class="space-y-0.5 ml-3 pl-3 border-l border-gray-200 dark:border-gray-700">
                        ...
                        <div v-if="sidebarCollapsed" class="space-y-0.5">
                            <Link v-for="item in group.items" :key="item.route" :href="route(item.route)" :title="item.label"
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 px-2 py-2 shrink-0">
                ...
                >
                    <span v-html="icons.settings" class="w-5 h-5 shrink-0" />
                ...
                >
                    <svg class="w-5 h-5 shrink-0 transition-transform duration-300" :class="sidebarCollapsed ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7" />
                    </svg>
                    <span v-if="!sidebarCollapsed" class="ml-3 truncate">Collapse</span>
                </button>
            </div>
        </aside>
            ...
            <header class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-50">
                    ...
                    <div class="flex items-center justify-between h-16">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div class="hidden md:flex items-center bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-1.5 w-48">
                                <svg class="w-4 h-4 text-gray-400 dark:text-gray-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                ...
                <div v-if="mobileMenuOpen" class="lg:hidden border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 max-h-[70vh] overflow-y-auto">
                    <div class="px-4 py-3 space-y-1">
                        ...
                        <template v-for="group in moreNavGroups" :key="group.label">
                            <button @click="toggleGroup(group.label)" class="flex items-center gap-2 w-full px-3 py-1 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">
                                <svg class="w-3 h-3 transition-transform duration-200" :class="expandedGroups.includes(group.label) ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                                {{ group.label }}
                            </button>
                            <div v-show="expandedGroups.includes(group.label)">
                                <Link v-for="item in group.items" :key="item.route" :href="route(item.route)" @click="mobileMenuOpen = false"
                                    class="flex items-center gap-3 pl-8 px-3 py-2 rounded-lg text-sm font-medium"
                                    :class="isActive(item.route) ? 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'">
                                    <span v-html="item.icon" class="w-4 h-4" />
                                    {{ item.label }}
                                </Link>
