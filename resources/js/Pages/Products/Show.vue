    ...
    <AppLayout :user="$page.props.auth.user">
        ...
        <div v-if="productImages.length" class="card p-6 mb-6">
            ...
            <div class="flex flex-wrap gap-4">
                <div v-for="(path, idx) in productImages" :key="idx" class="relative group cursor-pointer" @click="selectedImage = path">
                    <img :src="`/storage/${path}`" :alt="product.name" class="w-32 h-32 object-cover rounded-lg border border-gray-200 dark:border-gray-600 group-hover:ring-2 ring-accent transition-all" />
                </div>
            </div>
        </div>
        ...
        <div v-if="selectedImage" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70" @click.self="selectedImage = null">
            <div class="relative max-w-3xl max-h-[90vh] p-2">
                <img v-if="selectedImage" :src="`/storage/${selectedImage}`" :alt="product.name" class="max-w-full max-h-[85vh] rounded-lg object-contain" />
