<template>
  <AppLayout :user="$page.props.auth.user">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- Profile Avatar -->
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 flex flex-col items-center">
          <div class="relative group cursor-pointer mb-4" @click="$refs.profileAvatarInput.click()">
            <div v-if="userAvatar" class="w-28 h-28 rounded-full overflow-hidden border-4 border-gray-200 dark:border-gray-600">
              <img :src="userAvatar" class="w-full h-full object-cover" alt="Avatar" />
            </div>
            <div v-else class="w-28 h-28 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-blue-600 dark:text-blue-300 font-bold text-4xl border-4 border-gray-200 dark:border-gray-600">
              {{ user?.name?.charAt(0) || '?' }}
            </div>
            <div class="absolute inset-0 rounded-full bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
            </div>
            <input ref="profileAvatarInput" type="file" accept="image/*" class="hidden" @change="uploadAvatar" />
          </div>
          <p class="text-sm text-gray-500 dark:text-gray-400">Click to change photo</p>
          <p class="font-semibold text-gray-900 dark:text-gray-100 mt-2">{{ user?.name }}</p>
...
function uploadAvatar(event) {
  ...
  form.post(route('profile.avatar'), {
    onFinish: () => { if (profileAvatarInput.value) profileAvatarInput.value.value = '' }
  })
}
