<template>
    <AppLayout :user="$page.props.auth.user">
        <PageHeader :title="`Edit Payment ${payment.payment_number}`" />
        ...
        <form @submit.prevent="submit">
            <div class="card p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="file" accept="image/*,.pdf" @change="handleReceipt"
                        ...
                        <p v-if="payment.receipt_path" class="text-xs text-gray-400 dark:text-gray-500 mt-1">Current receipt uploaded. Upload a new file to replace it.</p>
                        <p v-else class="text-xs text-gray-400 dark:text-gray-500 mt-1">Upload image or PDF (max 5MB)</p>
            ...
            <div class="flex justify-end gap-3">
                <Link :href="route('payments.show', payment.id)" class="btn btn-outline">Cancel</Link>
...
const existingAllocations = props.payment.allocations || [];
const selectedInvoices = ref(existingAllocations.map(a => a.invoice_id));
const allocations = ref(
    existingAllocations.reduce((acc, a) => {
        acc[a.invoice_id] = parseFloat(a.amount);
        return acc;
    }, {})
);
